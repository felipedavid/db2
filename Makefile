network:
	sudo docker network create db_class
...
create:
	...
	sudo docker run --name postgres_db_class --network db_class -v ${HOME}/docker/db_class/postgresql:/var/lib/postgresql -v ${HOME}/docker/db_class/postgresql_data:/var/lib/postgresql/data -e POSTGRES_PASSWORD=postgres -p 5429:5432 -d postgres
